...
class MediaExtractor:
    """Extrae audio mono 16 kHz y frames a `detect_fps`.

    Para vídeos largos debe usarse `extract_window(...)`, que extrae solo una
    ventana temporal. `extract(...)` queda como compatibilidad y procesa el vídeo
    completo, pero no es recomendable para vídeos de 30-60 min.
    """
    ...
    def probe_duration(self, video_path: Path) -> float:
        """Duración del vídeo en segundos usando ffprobe."""
        cmd = [
            "ffprobe", "-v", "error",
    ...
    def extract_audio_segment(self, video_path: Path, video_id: str, start: float, duration: float, tag: str) -> Path:
        """Extrae audio de una ventana temporal del vídeo original."""
    ...
    def extract_frames(self, video_path: Path, video_id: str, overwrite: bool = True) -> List[Path]:
        """Extrae frames del vídeo completo. No recomendado para vídeos largos."""
    ...
    def extract_frames_segment(self, video_path: Path, video_id: str, start: float, duration: float,
        ...
        """Extrae frames de una ventana temporal.

        Los frames se guardan en temp/frames/<video_id>/<tag>/frame_XXXXXX.jpg y
        los tiempos son locales a la ventana: frame_idx / detect_fps.
        """
    ...
    def extract_window(self, video_path: Path, video_id: str, start: float, duration: float,
        ...
        """Extrae audio y frames de una ventana temporal del vídeo original."""

...
class IdentityManager:
    """Verificación de identidad facial por templates ArcFace/InsightFace."""
    ...
    def __init__(self, detector, celebrities, config):
        ...
        if bool(getattr(config, "use_face_classifier", False)):
            print("⚠️ use_face_classifier=true, pero no se ha proporcionado un clasificador entrenado.")
            print("   → Se usa verificación por templates para evitar predicciones aleatorias.")

        # Las identidades rivales son necesarias: un umbral absoluto no basta
        # para distinguir dos rostros parecidos. En reintentos selectivos solo
        # llega un famoso, por lo que descubrimos también el resto de referencias
        # por el prefijo ``<speaker_id>_``.
    ...
    def _build_templates(self, celebrities) -> None:
        for celeb in celebrities:
            ...
            if len(embeddings) >= self.min_template_images:
                ...
                # Medoide + inliers: una referencia equivocada no debe arrastrar
                # el template de identidad.
                ...
                print(f"⚠️ No se pudo crear template robusto para {getattr(celeb, 'id', 'unknown')}")
